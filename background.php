<?php
		...
		$userid=$rec1[0];
		$query2=mysqli_query($link,"select * from user_profile_pic where user_id='$userid'");
...
if(isset($_POST['file']) && ($_POST['file']=='Upload'))
{
		if($gender=="Male")
		{
			$path = "../../fb_users/Male/Profile/";
		}
		else
		{
			$path = "../../fb_users/Female/Profile/";
		}
		...
		if($gender=="Male")
		{
			move_uploaded_file($img_tmp_name,"../../fb_users/Male/Profile/".$prod_img_path);
		}
		else
		{
			move_uploaded_file($img_tmp_name,"../../fb_users/Female/Profile/".$prod_img_path);
		}
...
if(isset($_POST['file1']) && ($_POST['file1']=='Upload'))
{
		if($gender=="Male")
		{
			$path = "../../fb_users/Male/Cover/";
		}
		else
		{
			$path = "../../fb_users/Female/Cover/";
		}
		
		$img_name=rand().$_FILES['file1']['name'];
    	$img_tmp_name=$_FILES['file1']['tmp_name'];
    	$prod_img_path=$img_name;
		if($gender=="Male")
		{
			move_uploaded_file($img_tmp_name,"../../fb_users/Male/Cover/".$prod_img_path);
		}
		else
		{
			move_uploaded_file($img_tmp_name,"../../fb_users/Female/Cover/".$prod_img_path);
		}
...
if(isset($_POST['file2']) && ($_POST['file2']=='Upload'))
{
		...
		$img_name=rand().$_FILES['file2']['name'];
		...
		mysqli_query($link,"update user_cover_pic set image='$img_name' where user_id=$userid;");
		header("location:Profile.php");
}
?>
<html>
<head>
    ...
    <LINK REL="SHORTCUT ICON" HREF="../fb_title_icon/web.ico" />
...
<!--Head background-->
<div style="position:fixed;left:0;top:0; height:6%; width:100%; z-index:2; background:#0E4456">   </div>
<!--Head fb text-->
<div style="position:fixed;left:3.2%;top:0.8%;font-size:25;font-weight:900; z-index:3;"> <a href="../fb_home/Home.php" style="color:#FFFFFF; text-decoration:none;" onMouseOver="on_head_fb_text()" onMouseOut="out_head_fb_text()"> <font face="myFbFont"> blank</font> </a> </div>
<!--Head fb text background-->
<div style="position:fixed;left:4%;top:1%; height:5%; width:8%; z-index:2; background:#0E4456; display:none;" id="head_fb_text_backgraound">   </div>
...
<form name="fb_search" action="../fb_home/Search_Display_submit.php" method="get" onSubmit="return bcheck()">
	<div style="position:fixed; left:19%; top:1.2%; z-index:2;"> <input type="text" name="search1" placeholder="Search for people On blank" style="height:25; width:200;" onKeyUp="searching();" id="search_text1"> </div>
	
	<div id="searching_ID"></div> 
	
	<div style="position:fixed; left:44.2%;top:1.6%; z-index:2;">
...
<div style="position:fixed; left:71.8%; top:0.6%; z-index:2;">
	...
	<tr id="hedarname2">
		...
		<td style="padding-left:7;" id="head_img" onMouseOver="head_pro_pic_over()" onMouseOut="head_pro_pic_out()"> <a href="Profile.php">  <img src="../../fb_users/<?php echo $gender; ?>/Profile/<?php echo $img; ?>" style="height:27; width:25;"> </a> </td>
...
<div style="position:fixed; left:97%; top:0.4%; z-index:2;"> <img src="background_file/background_icons/nexusae0_home_settings_icon2.png" height="35" width="35" onClick="close_option()"> </div>
        ...
        <div style="position:fixed; left:85%; top:6%; z-index:3; background:#FFF; height:39%; width:14.8%; box-shadow:0px 2px 10px 1px rgb(0,0,0);"> </div>
...
<div style="position:fixed; left:86.1%; top:30.5%; z-index:3;"> <a href="../fb_home/feedback.php"> <img src="background_file/background_icons/icon-feedback.png" height="20" width="20" onMouseOver="head_feedback_over()" onMouseOut="head_feedback_out()"> </a> </div>
<div style="position:fixed; left:88.3%; top:27.5%; z-index:3;"><a href="../fb_home/feedback.php" style="text-decoration:none; color:#000;" id="head_feedback" onMouseOver="head_feedback_over()" onMouseOut="head_feedback_out()"><h4> Feedback </h4></a></div>

<div style="position:fixed; left:86%; top:35.5%; z-index:3;"> <a href="../fb_logout/logout.php"> <img src="background_file/background_icons/logout.png" height="20" width="20" onMouseOver="head_logout_over()" onMouseOut="head_logout_out()"> </a> </div>
<div style="position:fixed; left:88.3%; top:31.1%; z-index:3;"><a href="../fb_logout/logout.php" style="text-decoration:none; color:#000;" id="head_logout" onMouseOver="head_logout_over()" onMouseOut="head_logout_out()"><h4> Logout </h4></a></div>
</div>
...
<!--cover img-->
<?php 
	$query3=mysqli_query($link,"select * from user_cover_pic where user_id=$userid");
	$rec3=mysqli_fetch_array($query3);
	$cover_img=$rec3[2];
	
	$que_post_bg=mysqli_query($link,"select * from user_post where user_id=$userid");
	$count_bg=mysqli_num_rows($que_post_bg);
	$count_bg=$count_bg+1;
?>
<div style="position:absolute; left:15.11%; top:6%;"> <img src="../../fb_users/<?php echo $gender; ?>/Cover/<?php echo $cover_img; ?>" height="279" width="943" onMouseOver="dis_cover_pic_edit();" onMouseOut="out_cover_pic_edit();" onClick="open_cover_photo()"> </div>
...
<img src="../../fb_users/<?php echo $gender; ?>/Cover/<?php echo $cover_img; ?>" style="height:500; width:800; ">
</div>
</div>
...
<div style="position:absolute; left:16.5%; top:33%; z-index:1;">
	<table border="0">
	<tr>
		<td>  <img src="../../fb_users/<?php echo $gender; ?>/Profile/<?php echo $img; ?>" style="height:150; width:150;" onMouseOver="dis_pro_pic_edit();" onMouseOut="out_pro_pic_edit();" onClick="open_profile_photo()">  </td>
...
<div style="position:fixed; background:#FFF; left:17%; top:5%; height:90%; width:65.5%; z-index:3"></div>
<div style="position:fixed; left:20%; top:10%;z-index:4;">
<img src="../../fb_users/<?php echo $gender; ?>/Profile/<?php echo $img; ?>" style="height:500; width:800; ">
</div>
</div>
...
<div style="position:fixed; left:84%; top:6%; background-color:#0E4456;  height:89.2%; width:16%; z-index:2;"></div>
...
<?php
	 while($online_data=mysqli_fetch_array($query_online))
	 {
		...
		$query_online_user_data=mysqli_query($link,"select * from users where user_id=$online_user_id");
		$query_online_user_pic=mysqli_query($link,"select * from user_profile_pic where user_id=$online_user_id");
...
?>
			 <tr>
			   	   <td> <a href="../fb_view_profile/view_profile.php?id=<?php echo $online_user_id; ?>"> <img src="../../fb_users/<?php echo $online_user_gender; ?>/Profile/<?php echo $online_user_pic; ?>" height="30" width="30"> </a> </td>
